...
int main()
{
	...
	while(1)
	{
		...
		while(I2C_MasterSendDataIT(&I2C1_Handle,&commandCode,1,SLAVE_ADDR,0) !=I2C_READY);


		while(I2C_MasterReceiveDataIT(&I2C1_Handle,&NBytes,1, SLAVE_ADDR,0) !=I2C_READY);

		commandCode=0x52;
		//send 0x52 to read number of bytes in read phase
		while(I2C_MasterSendDataIT(&I2C1_Handle,&commandCode,1,SLAVE_ADDR,0) !=I2C_READY);

		// do another read to read those Nbytes of Data
		while(I2C_MasterReceiveDataIT(&I2C1_Handle,rcvBuffer,NBytes,SLAVE_ADDR,0) !=I2C_READY);
